...
export default function Repositories() {
  ...
  const getRepositories = async () => {
    fetch("https://api.github.com/users/michelfawazz" + "/repos")
      .then((response) => response.json())
      .then((data) => {
        setData(
          data
            .filter((d) => d.fork === false)
            .sort((a, b) => {
  ...
  return (
    <div id="repo-component">
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 w-full gap-2 items-center mt-4">
          {data.map((item, index) => (
            <Link key={index} href={"https://github.com/michelfawazz/" + item.name} target="_blank" className="h-full bg-zinc-900/90 hover:bg-zinc-200/20 relative transition-all duration-200 flex flex-col rounded-lg py-4 px-5  w-11/12 mx-auto">
              <i className="fal fa-arrow-up-right-from-square absolute top-4 right-5" />
              <h1 className="leading-none text-lg text-zinc-100">
                <span className="text-primary text-xl font-medium">Michel</span>/{item.name}
              </h1>
